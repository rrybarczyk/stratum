...
impl DownstreamMiningNode {
    ...
    pub async fn next(self_mutex: Arc<Mutex<Self>>, mut incoming: StdFrame) {
        ...
        match next_message_to_send {
            ...
            // Panic here is fine cause RelayNewMessage is used only in the setup connection phase
            Ok(SendTo::RelayNewMessage(_)) => panic!(),
            Ok(SendTo::None(_)) => (),
    ...
    > for DownstreamMiningNode
{
    fn handle_setup_connection(
        ...
        Ok(SendToCommon::RelayNewMessage(message.try_into().unwrap()))
    }
}
    ...
    while let Ok((stream, _)) = listner.accept().await {
        ...
        task::spawn(async move {
            ...
            match DownstreamMiningNode::handle_message_common(
                ...
                Ok(SendToCommon::RelayNewMessage(message)) => {
